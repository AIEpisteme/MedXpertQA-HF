...
if __name__ == '__main__':
    ...
    parser.add_argument("--provider", default='api', choices=["api", "hf"], type=str)
    ...
    parser.add_argument("--hf-device-map", default="auto", type=str)
    parser.add_argument("--hf-torch-dtype", default="auto", type=str)
    parser.add_argument("--hf-max-new-tokens", default=None, type=int)
    parser.add_argument("--hf-max-input-tokens", default=None, type=int)
    parser.add_argument("--hf-trust-remote-code", default=False, action='store_true')
    parser.add_argument("--hf-local-files-only", default=False, action='store_true')
    parser.add_argument("--hf-revision", default=None, type=str)
    parser.add_argument("--hf-cache-dir", default=None, type=str)
    parser.add_argument("--hf-attn-implementation", default=None, type=str)
    parser.add_argument("--hf-image-policy", default="error", choices=["error", "ignore", "describe"], type=str)
    parser.add_argument("--hf-token-env", default="HF_TOKEN", type=str)
    args = parser.parse_args()

    if args.provider == "hf" and args.num_threads != 1:
        logging.warning("Hugging Face local inference uses one loaded model instance; forcing --num-threads 1.")
        args.num_threads = 1
    ...
    if args.provider == "hf":
        model_kwargs = {
            ...
            "image_policy": args.hf_image_policy,
            "token_env": args.hf_token_env,
            "seed": args.seed,
        }

    llm_agent, tasks = setup(
        args.model,
        args.dataset,
        args.method,
        args.prompting_type,
