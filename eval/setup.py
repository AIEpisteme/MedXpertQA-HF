import os
import json
from model import APIAgent, HuggingFaceAgent

# Constants
METHOD_LIST = ["zero_shot", "few_shot"]
DEMO_TYPE_LIST = ["ao", "cot"]
PROVIDER_LIST = ["api", "hf"]
...
def load_model(model, model_info, provider="api", **model_kwargs):
    if provider == "hf":
        return HuggingFaceAgent(model, **model_kwargs)

    API_MODEL = model_info.get("API_MODEL", [])
    if provider == "api" and model in API_MODEL:
        return APIAgent(model)
    else:
        raise ValueError(f"Unknown model/provider combination: provider={provider}, model={model}")

# Setup function refactored to optimize config loading and checks
def setup(model, dataset, method, demo_type, provider="api", **model_kwargs):
    ...
    if provider not in PROVIDER_LIST:
        raise ValueError(f"Undefined provider {provider} in PROVIDER_LIST")
    ...
    model = load_model(model, model_info, provider, **model_kwargs)
